...
def user():
    ...
    for item in subResponse['Items']:
        # musResponse.update(music.scan(FilterExpression=Attr('title').eq(item['Song'])))
        musResponse['Items'].append(music.scan(FilterExpression=Attr('title').eq(item['Song'])))
    # return render_template('user.html', music=musResponse['Items'])
    return musResponse['Items']
